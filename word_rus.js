...
const wordsLength = data.length
const random = (value) => Math.floor(Math.random() * value)
const getWord = () => data[random(wordsLength - 1)]
textEl.textContent = getWord()
const innerText = () => textEl.textContent = getWord()
selectTime.addEventListener('change', onChangeTimerValue)
let int = setInterval(() => innerText(), 1000 * time)
function onChangeTimerValue () {
    time = document.querySelector('#time').value;
    clearInterval(int)
    int = setInterval(() => innerText(), 1000 * time)
}
nextBtn.onclick = () => {textEl.textContent = getWord(); onChangeTimerValue()}